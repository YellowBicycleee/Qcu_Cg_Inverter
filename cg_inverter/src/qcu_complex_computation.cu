...
// DESCRIBE：  x = ax
static __global__ void sclar_multiply_vector_gpu(void *x, const Complex a,
                                                 int vol) {
  // a : complex
  int pos = blockIdx.x * blockDim.x + threadIdx.x; // thread pos in total
  Complex scalar = a; //*(static_cast<Complex *>(a));
...
static __global__ void saxpy_gpu(void *y, void *x, const Complex a, int vol) {
  // a : complex
  int pos = blockIdx.x * blockDim.x + threadIdx.x; // thread pos in total
  Complex scalar = a; //*(static_cast<Complex *>(a));
...
static __global__ void partial_product_kernel(void *x, void *y,
                                              void *partial_result, int vol) {
...
static __global__ void reduce_partial_result(void *partial_result,
                                             int partial_length) {
  int thread_in_block = threadIdx.x;
  int stride, last_stride;
...
__global__ void norm2_gpu(void *vector, void *partial_result, int vector_length,
                          void *result) {
...
void gpu_inner_product(void *x, void *y, void *result, void *partial_result,
  ...
  checkCudaErrors(cudaDeviceSynchronize());
  checkCudaErrors(cudaMemcpy(result, partial_result, sizeof(Complex),
...
void gpu_inner_product_new(void *x, void *y, void *result, void *partial_result,
  ...
  partial_inner_prod_kernel<<<grid_size, block_size>>>(x, y, partial_result,
                                                       vector_length);
  // checkCudaErrors(cudaDeviceSynchronize());
  reduce_partial_result<<<1, block_size>>>(partial_result, grid_size);
  checkCudaErrors(cudaDeviceSynchronize());
