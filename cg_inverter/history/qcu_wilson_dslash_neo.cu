...
static __global__ void dslashNaiveKernel(void *gauge, void *fermion_in, \
  ...
  coord_boundary = (grid_z > 1) ? Lz-1 : Lz;
  if (z < coord_boundary) {
    spinor_gauge_mul_add_vec<Z_DIRECTION, FRONT> (u_local, src_local, dst_local, \
                                                  flag);
  }
  ...
  coord_boundary = (grid_z > 1) ? 1 : 0;
  if (z >= coord_boundary) {
    spinor_gauge_mul_add_vec<Z_DIRECTION, BACK> (u_local, src_local, dst_local, \
                                                  flag);
  }
  ...
  coord_boundary = (grid_t > 1) ? Lt-1 : Lt;
  if (t < coord_boundary) {
    spinor_gauge_mul_add_vec<T_DIRECTION, FRONT> (u_local, src_local, dst_local, \
                                                  flag);
  }
  // t: back
